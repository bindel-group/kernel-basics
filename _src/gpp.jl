...
removing points and changing the kernel.
...
And now we check consistency of the predictive mean and variance and
their derivatives.
...
Now we check that the predictive mean and variance agree with our
earlier calculations without the GPP context object.
