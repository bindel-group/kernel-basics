...
@testset "Kernel hyper derivatives" begin
    ...
    ℓ = 0.2996
    kse(ℓ) = KernelSE{2}(ℓ)
    @test gθ_kernel(kse(ℓ),x,y)[1] ≈
        diff_fd(s->kernel(kse(ℓ+s),x,y)) rtol=1e-6
    @test Hθ_kernel(kse(ℓ),x,y)[1,1] ≈
        diff_fd(s->gθ_kernel(kse(ℓ+s),x,y)[1]) rtol=1e-6
end
...
@testset "RQ kernel hyper derivatives" begin
    ...
    @test gθ_kernel(k(ℓ,α),x,y)[1] ≈ diff_fd(s->kernel(k(ℓ+s,α),x,y)) rtol=1e-6
    @test gθ_kernel(k(ℓ,α),x,y)[2] ≈ diff_fd(s->kernel(k(ℓ,α+s),x,y)) rtol=1e-6
    @test Hθ_kernel(k(ℓ,α),x,y)[:,1] ≈
        diff_fd(s->gθ_kernel(k(ℓ+s,α),x,y)) rtol=1e-6
    @test Hθ_kernel(k(ℓ,α),x,y)[:,2] ≈
        diff_fd(s->gθ_kernel(k(ℓ,α+s),x,y)) rtol=1e-6
end

@testset "Kernel spatial derivatives" begin
    x, y, dx = [0.1; 0.2], [0.8; 0.8], [0.617; 0.779]
    k = KernelM5{2}(0.5)
    @test gx_kernel(k,x,y)'*dx ≈ diff_fd(x->kernel(k,x,y),x,dx) rtol=1e-6
    @test Hx_kernel(k,x,y)*dx ≈ diff_fd(x->gx_kernel(k,x,y),x,dx) rtol=1e-6
end
